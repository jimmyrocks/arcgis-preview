...
function buildZip(files) {
    ...
    for (const r of records) {
        ...
        writeU16(out, off, 0);
        off += 2; // compression method: 0 (stored)
        writeU16(out, off, r.mtime);
        off += 2; // last mod file time
        writeU16(out, off, r.mdate);
        off += 2; // last mod file date
        writeU32(out, off, r.crc >>> 0);
        off += 4;
        writeU32(out, off, r.data.length);
        off += 4; // compressed size
        writeU32(out, off, r.data.length);
        off += 4; // uncompressed size
        writeU16(out, off, r.nameBytes.length);
        off += 2; // file name length
        writeU16(out, off, 0);
        off += 2; // extra field length
        out.set(r.nameBytes, off);
        off += r.nameBytes.length;
        out.set(r.data, off);
        off += r.data.length;
    }
    ...
    for (const r of records) {
        writeU32(out, off, 0x02014b50);
        off += 4; // central file header signature
        writeU16(out, off, 20);
        ...
        writeU16(out, off, 0);
        off += 2; // disk number start
        writeU16(out, off, 0);
        off += 2; // internal file attributes
        writeU32(out, off, 0);
        off += 4; // external file attributes
        writeU32(out, off, r.localHeaderOffset);
    ...
    writeU32(out, off, 0x06054b50);
    off += 4;
    writeU16(out, off, 0);
    off += 2; // number of this disk
    writeU16(out, off, 0);
    off += 2; // disk with central dir start
    writeU16(out, off, records.length);
    off += 2; // number of entries on this disk
    writeU16(out, off, records.length);
    off += 2; // total number of entries
    writeU32(out, off, centralDirSize);
    off += 4; // size of central directory
    writeU32(out, off, centralDirOffset);
    off += 4; // offset of central directory
    writeU16(out, off, 0);
    off += 2; // .ZIP file comment length
    return out;
}

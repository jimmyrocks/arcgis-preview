...
exports.buildLayerUrl = buildLayerUrl;
exports.formatDate = formatDate;
exports.timeExtentToBadge = timeExtentToBadge;
exports.computeLastEditDateString = computeLastEditDateString;
function buildLayerUrl(serviceUrl, id) {
    if (!serviceUrl || id === undefined || id === null)
        return null;
    const trimmed = serviceUrl.replace(/\/+$/, '');
    if (/\/(\d+)$/.test(trimmed))
        return trimmed; // already a layer URL
    if (/\/(MapServer|FeatureServer)$/i.test(trimmed))
        return `${trimmed}/${id}`;
    return null;
}
...
function timeExtentToBadge(timeInfo) {
    try {
...
async function computeLastEditDateString(layerMeta, serviceUrl) {
    try {
        if (!layerMeta)
            return '';
        const editingInfo = layerMeta?.editingInfo;
        const editFieldsInfo = layerMeta?.editFieldsInfo;
        if (editingInfo && typeof editingInfo?.lastEditDate === 'number') {
            return formatDate(new Date(editingInfo.lastEditDate));
        }
        const editField = editFieldsInfo?.editDateField || editFieldsInfo?.lastEditDateField || editFieldsInfo?.editDateFieldName;
        if (editField) {

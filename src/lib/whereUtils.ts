export function normalizeWhereInput(s: string): string {
  try {
    let out = String(s || '').trim();
    out = out.replace(/;+\s*$/g, '').trim();
    if (/^where\b/i.test(out)) out = out.replace(/^where\b/i, '').trim();
    return out;
  } catch {
    return String(s || '');
  }
...
export function validateWhere(
  ...
  if (/^\s*where\b/i.test(trimmed)) warnings.push('Only enter the WHERE clause. "WHERE" is optional and will be stripped on apply.');
  if (/\bselect\b/i.test(trimmed) || /\bfrom\b/i.test(trimmed)) errors.push('Only enter the WHERE clause, not a full SELECT statement');
  if (/;+\s*$/.test(trimmed)) warnings.push('Trailing semicolons are ignored');
  ...
  const keywordSet = new Set(['AND','OR','NOT','LIKE','IN','BETWEEN','IS','NULL','TRUE','FALSE','WHERE','SELECT','FROM','ORDER','GROUP','BY','DATE','TIMESTAMP','CURRENT_TIMESTAMP']);

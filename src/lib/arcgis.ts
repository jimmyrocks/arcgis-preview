export type restServiceUrlInfo = {
  ...
  serviceType: 'MapServer' | 'FeatureServer' | 'ImageServer' | 'VectorTileServer' | 'GPServer' | 'NAServer' | 'GeometryServer' | 'GeocodeServer' | 'NetworkAnalysisServer' | 'GlobeServer' | 'SceneServer' | null;
...
export function canonicalizeArcgisRestUrl(url: string | URL): string {
...
export function coerceArcgisRestServicesUrl(url: string | URL): string | null {
  let urlObj: URL;
  try {
  ...
  const restIndex = segs.findIndex((segment) => segment.toLowerCase() === 'rest');
  if (restIndex < 0) return null;
  if (segs[restIndex + 1]?.toLowerCase() !== 'services') {
    segs.splice(restIndex + 1, 0, 'services');
    urlObj.pathname = `/${segs.join('/')}`;
  }
  return canonicalizeArcgisRestUrl(urlObj);
}

export function buildArcgisJsonUrl(baseUrl: string | URL, pathSuffix: string = ''): string {
  const urlObj = new URL(typeof baseUrl === 'string' ? baseUrl : baseUrl.toString());
  const basePath = urlObj.pathname.replace(/\/+$/, '');
  const suffix = pathSuffix ? `/${pathSuffix.replace(/^\/+|\/+$/g, '')}` : '';
  urlObj.pathname = `${basePath}${suffix}` || '/';
...
export function getRestServiceUrlInfo(url: string | URL): restServiceUrlInfo {
  ...
  const validTypes = new Set([
    'MapServer', 'FeatureServer', 'ImageServer', 'VectorTileServer', 'GPServer', 'NAServer', 'GeometryServer', 'GeocodeServer', 'NetworkAnalysisServer', 'GlobeServer', 'SceneServer'
  ]);

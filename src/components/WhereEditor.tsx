...
/**
 * Lightweight SQL WHERE editor with syntax highlighting.
 * Uses CodeMirror 6 with a minimal setup and app theme colors.
 */
const WhereEditor = React.forwardRef<WhereEditorHandle, Props>(function WhereEditor({ value, placeholder = '1=1', onChange, onCommit, height = '32px', wrap = false, fields = [], fieldsMeta = [], valueSamples = {}, keywords, fieldAliases = {}, commitKey = 'enter', readOnly = false, ...rest }: Props, ref) {
  ...
  const extensions = React.useMemo(() => {
    ...
    // Capture the view instance
    try { exts.push(EditorView.updateListener.of((vu) => { try { viewRef.current = vu.view; } catch {} })); } catch {}
    // Minimal autocompletion: fields, simple operators/keywords, and optional sample values
    ...
    const fieldComps: Completion[] = (fields || []).map((f) => {
      ...
      // Give fields a boost so they appear before short keywords like OR
    ...
    const getValueOptions = (field?: string, quotedAlready: boolean = false): Completion[] => {
      ...
      try {
        ...
        if (field && Array.isArray((valueSamples as any)[field])) {
          ...
          for (let i = 0; i < arr.length && i < maxVals; i++) add(arr[i], preferNumeric);
        } else {
          for (const k of Object.keys(valueSamples || {})) {
            const arr = Array.isArray((valueSamples as any)[k]) ? ((valueSamples as any)[k] as unknown[]) : [];
            for (let i = 0; i < arr.length && i < maxVals; i++) add(arr[i], typeof arr.find((v: any) => v != null) === 'number');
          }
        }
      } catch {}
      return out;
    ...
    const source = (ctx: CompletionContext) => {
      const before = ctx.matchBefore(/[\w$]+|[=<>!]+|\b(?:IS\s+NOT\s+NULL|IS\s+NULL)\b/i);
      if (!before && !ctx.explicit) return null;
      const token = before ? before.text.toUpperCase() : '';

      // Heuristic: if previous tokens match "<field> <operator>", suggest values for that field
      const line = ctx.state.doc.lineAt(ctx.pos);
      const upto = line.text.slice(0, ctx.pos - line.from);
      const m = upto.match(/([A-Za-z_][\w\.]*)\s*(?:=|<>|<=|>=|<|>|\bLIKE\b|\bIN\b|\bBETWEEN\b)\s*$/i);
      if (m) {
        const fld = m[1];
        const prevChar = (ctx.state.doc.sliceString(Math.max(0, ctx.pos - 1), ctx.pos) || '');
        const quotedAlready = prevChar === "'"; // cursor right after opening quote
        const options = getValueOptions(fld, quotedAlready);
        if (options.length) return { from: before?.from ?? ctx.pos, options };
      }
      // Fallback: if cursor appears after any operator, show generic values
      const prev2 = ctx.state.sliceDoc(Math.max(0, (before?.from ?? ctx.pos) - 2), before?.from ?? ctx.pos);
      if (/[=<>!]$/.test(prev2 || '')) {
        const prevChar = (ctx.state.doc.sliceString(Math.max(0, ctx.pos - 1), ctx.pos) || '');
        const quotedAlready = prevChar === "'";
        const options = getValueOptions(undefined, quotedAlready);
        if (options.length) return { from: before?.from ?? ctx.pos, options };
      }

      // If token looks like identifier, suggest fields and keywords
    ...
    // Lightweight linter: unmatched quotes/parentheses and unknown fields
    ...
    const whereLinter = linter((view) => {
      ...
      // Parentheses and quotes tracking
      const stack: number[] = [];
      let inStr = false;
      let strStart = -1;
      ...
      // Unknown field names (case-insensitive), ignore inside strings
      // Simple scan for identifiers
      ...
      while (i < text.length) {
        ...
        if (!inside && isIdentStart(ch)) {
          ...
          if (!fieldSet.has(upper) && !keywordSet.has(upper) && !/^(?:TRUE|FALSE|NULL)$/.test(upper)) {
            // Avoid flagging numbers or table-less SQL tokens; warn only
      ...
      // Numeric/OID misuse underline (quoted numeric, LIKE on numeric)
      try {
        ...
        while ((m = re.exec(text)) != null) {
          ...
          if (op === 'LIKE') {
            diags.push({ from: m.index, to: m.index + matchStr.length, severity: 'error', message: `Numeric field ${field} used with LIKE; likely invalid` });
          }
          if (/^'.*'$/.test(valTrim)) {
            diags.push({ from: valueFrom, to: valueTo, severity: 'error', message: `Numeric field ${field} compared to quoted value; remove quotes` });
          }
        }
      } catch { }
      // Flag unsupported operator "!=" (ESRI uses <> instead)
      try {
        let idx = text.indexOf('!=');
        while (idx !== -1) {
          diags.push({ from: idx, to: idx + 2, severity: 'error', message: '!= is not supported; use <>' });
  ...
  return (
    <div style={{ flex: '1 1 auto' }}>
      <CodeMirror
        ...
        // Keep height compact like the previous input
        height={height}
        style={{
          fontSize: 13,
          borderRadius: 8,
          border: '1px solid var(--border)',
          // Allow autocomplete tooltip to overflow the editor box
          overflow: 'visible',
        }}
        // Do not commit on blur; user applies explicitly via Enter or button
        {...rest}
      />
    </div>
  );

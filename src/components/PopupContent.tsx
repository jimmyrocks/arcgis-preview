...
export default function PopupContent({ feature, layerName, maxWidth = 340, maxHeight = 240, headerStart, headerEnd }: PopupContentProps) {
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 6, padding: '8px 10px', borderBottom: '1px solid #2b3050', background: '#12162a' }}>
        {headerStart ? <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>{headerStart}</div> : null}
        ...
        {headerEnd ? <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginLeft: 'auto' }}>{headerEnd}</div> : null}
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
            <tbody>
              {entries.length === 0 ? (
                ...
                <tr key={k}>
                  <th style={{ textAlign: 'left', padding: '4px 8px', whiteSpace: 'nowrap', color: '#e6e8ef', borderTop: '1px solid #1b2238', width: '38%', maxWidth: '38%' }}>{k}</th>
                  <td style={{ padding: '4px 8px', color: '#c3c7d5', borderTop: '1px solid #1b2238', wordBreak: 'break-word' }}>{formatAttrValue(v)}</td>
                </tr>
              ))}
            </tbody>
          </table>

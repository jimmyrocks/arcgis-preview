...
GeoJSON vs vector-tile experiment

MapLibre already converts a GeoJSON source into render tiles in a worker. The
experimental comparison page tests a different boundary: sending the complete
FeatureCollection to that worker versus exposing lazily generated MVT bytes
through a custom-protocol vector source.

After starting the app, open:

`http://localhost:5173/?experiment=geojson-vs-tiles&features=10000`

The two synchronized maps use the same synthetic dataset and report:

- serialized GeoJSON bytes and stringify time;
- client-side MVT index-build time;
- vector-tile requests, cache hits, generation time, and generated bytes;
- rendered feature counts for correctness; and
- exact unsafe-ID behavior through string-property `promoteId`.

Change `features` between 10 and 100000 and pan/zoom the maps. This is test
scaffolding, not a production source mode. It intentionally makes the
client-side MVT encoding cost visible. A production tile mode would move tile
generation behind a worker, service, or cache and would need an invalidation
contract for incremental ArcGIS updates.
